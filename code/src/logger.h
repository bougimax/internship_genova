...
inline void startLogging(const char *fn) {
  if (fn != NULL) {
    ...
    if (log_fp == NULL) {
      ...
      fprintf(log_fp,
              "Input_File, Delaunay(ms), Segment_Rec(ms), Face_Rec(ms), "
              "Erosion(ms), Memory(MB), Is_Polyhedron, Input_Verts, "
              "Input_Tris, Steiner, Tot_Tets, In_Tets, Flat_Tets, "
              "Flipped_Tets, Si's method works\n");
    } else {
      fclose(log_fp);
      log_fp = fopen("cdt_log.csv", "a");
    }
    if (log_fp == NULL)
      ip_error("Can't open cdt_log.csv for logging!\n");
...
inline void logTimeChunk() {
  ...
  uint64_t ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(now - time_point)
          .count();
  time_point = now;
...
inline void logInteger(uint32_t n) { fprintf(log_fp, ", %u", n); }
...
// Mem info in Mb
inline void logMemInfo() { fprintf(log_fp, ", %.2f", getPeakMegabytesUsed()); }
#else
