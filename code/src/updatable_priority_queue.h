...
template <typename Key, typename Priority> struct priority_queue_node {
  Priority priority;
  Key key;
  priority_queue_node(const Key &key, const Priority &priority)
...
template <typename Key, typename Priority> class updatable_priority_queue {
protected:
...
public:
   ...
   * */
  bool set(const Key &key, const Priority &priority,
           bool only_if_higher = false) {
