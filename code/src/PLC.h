...
// NOTES: 1) "both_acute_ep" edges will be immediatelly split by inserting the
// middle point (each subedge becomes a "one_acute_ep")
//        2) sub-edges of "no_acute_ep" and "one_acute_ep" inherit type
//        3) "flat" edges will be ignored by segment recovery algorithm and will
...
public:
  ...
  uint32_t oep[2]; // Parent-edge endpoints (vertices-inds wrt TetMesh vertices
                   // vector)
  std::vector<uint32_t>
      inc_tri; // Incident triangles (triangle-inds wrt input triangles vector)
  PLCedge_type type; // See notes after definition of PLCedge_type.

  inline PLCedge() {}

  inline PLCedge(const uint32_t e0, const uint32_t e1, const uint32_t oe0,
                 const uint32_t oe1, const std::vector<uint32_t> &itri,
                 const PLCedge_type t)
      : ep{e0, e1}, oep{oe0, oe1}, inc_tri(itri), type(t) {}

  bool isFlat() const { return type == flat; }

  inline void swap() {
    std::swap(ep[0], ep[1]);
    std::swap(oep[0], oep[1]);
  }

  inline void fill_preEdge(const uint32_t v0, const uint32_t v1,
                           const uint32_t fi) {
    if (v0 > v1) {
      oep[0] = ep[0] = v1;
      oep[1] = ep[1] = v0;
  ...
  uint32_t commonVertex(const PLCedge &e) const {
    if (ep[0] == e.ep[0] || ep[0] == e.ep[1])
      return ep[0];
    else if (ep[1] == e.ep[0] || ep[1] == e.ep[1])
      return ep[1];
    else
      return UINT32_MAX;
  }
  ...
  bool coincident(const PLCedge &e) const {
    return (ep[0] == e.ep[0] && ep[1] == e.ep[1]) ||
           (ep[0] == e.ep[1] && ep[1] == e.ep[0]);
  }
  ...
  uint32_t commonOriginalVertex(const PLCedge &e) const {
    if (oep[0] == e.oep[0] || oep[0] == e.oep[1])
      return oep[0];
  ...
  static inline bool vertexSortFunc(const PLCedge &e1, const PLCedge &e2) {
    if (e1.ep[0] == e2.ep[0])
      return (e1.ep[1] < e2.ep[1]);
    else
      return (e1.ep[0] < e2.ep[0]);
  }
...
public:
  ...
  std::vector<std::pair<uint32_t, uint32_t>>
      orig_flat_edges; // Original flat edges
  std::vector<uint32_t>
      vertices; // Ordered mesh vertices bounding the face (see savePLC)
  std::vector<uint32_t>
      flat_vertices;   // Face internal vertices (having a flat neghborhood)
  ...
  void replaceIncidentEdgeFaces(uint32_t old_f, uint32_t new_f) {
    for (PLCedge *e : bounding_edges)
      e->replaceIncidentFace(old_f, new_f);
  }

  static inline bool isEmpty(const PLCface &e) {
    return e.bounding_edges.empty();
  }
};
...
public:
  ...
  std::vector<std::vector<std::vector<uint32_t>>>
      vt_maps; // Set of input triangles incident upon each vertex

  std::vector<PLCface> faces;      // Faces of the PLC
  std::vector<uint32_t> v_reindex; // Maps global vertex IDs to local indexes
                                   // into face vertex vectors
  std::vector<std::pair<uint32_t, uint32_t>>
      singular_v; // Set of pairs <global_vertex_ID, local_face_vertex_ID>, one
                  // per singular vertex in face
  ...
  void makeVertexTriangleMaps(
      std::vector<std::vector<std::vector<uint32_t>>> &vt_maps);

  void makeVertexTriangleMap(PLCface &f,
  ...
  bool isFlat(const PLCedge &e) const {
    return e.inc_tri.size() == 2 &&
           delmesh.vOrient3D(e.ep[0], e.ep[1], opposite_vrt(e, e.inc_tri[0]),
                             opposite_vrt(e, e.inc_tri[1])) == 0;
  }
  ...
  inline implicitPoint_LNC *getProjectionOrMidPoint(uint32_t oe0i,
                                                    uint32_t oe1i, uint32_t e0i,
                                                    uint32_t e1i, uint32_t ri,
                                                    uint32_t &acute_v) const;
  inline implicitPoint_LNC *
  getProjectionOrMidPoint_noac(uint32_t oe0i, uint32_t oe1i, uint32_t e0i,
                               uint32_t e1i, uint32_t ri,
                               uint32_t &acute_v) const;
  inline implicitPoint_LNC *
  getProjectionOrMidPoint_noac_rev(uint32_t oe0i, uint32_t oe1i, uint32_t e0i,
                                   uint32_t e1i, uint32_t ri,
                                   uint32_t &acute_v) const;
  inline implicitPoint_LNC *getMidPoint(uint32_t oe0i, uint32_t oe1i,
  ...
  void
  segmentRecovery_HSi(bool quiet = false); // Segment recovery main function
  ...
  int localOrient3d(uint32_t v1, uint32_t v2, uint32_t v3, uint32_t v4,
                    std::vector<uint32_t> &to_unorient);
  int cachedOrient3D(uint32_t v, uint32_t v1, uint32_t v2, uint32_t v3);

  bool recoverFaceHSi(std::vector<uint64_t> &i_tets, const PLCface &f,
                      bool &sisMethodWorks);

  void giftWrap(std::vector<uint64_t> &bnd,
                const std::vector<uint32_t> &vertices,
                std::vector<uint32_t> &newtets);
  uint64_t missingFaceInCavity(const std::vector<uint64_t> &bnd,
                               const std::vector<uint32_t> &vertices);
  uint64_t meshCavity(const std::vector<uint64_t> &bnd,
                      const std::vector<uint32_t> &vertices,
                      std::vector<uint64_t> &base);
  uint64_t expandCavity(std::vector<uint64_t> &bnd,
                        std::vector<uint32_t> &vertices, uint64_t t,
                        const PLCface &f);

  size_t markInnerTets();

  // void getTetsIntersectingFaceSlow(uint32_t fi, std::vector<uint64_t>*
  // i_tets) {
  ...
  //    delmesh.marked_vertex[v] = 0; return;
  //}
