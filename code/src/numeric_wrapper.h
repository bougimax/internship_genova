...
public:
  ...
  static int orient2D(const coord_t &p0, const coord_t &p1, const coord_t &q0,
                      const coord_t &q1, const coord_t &r0, const coord_t &r1) {
#ifdef USE_DOUBLE
  ...
  static int inSphere(const pointType &pa, const pointType &pb,
                      ...
                      const pointType &pe) {
#ifdef USE_DOUBLE
    return ::inSphere(pa.X(), pa.Y(), pa.Z(), pb.X(), pb.Y(), pb.Z(), pc.X(),
                      pc.Y(), pc.Z(), pd.X(), pd.Y(), pd.Z(), pe.X(), pe.Y(),
                      pe.Z());
#else
    const coord_t aex = pa[0] - pe[0], bex = pb[0] - pe[0], cex = pc[0] - pe[0],
                  dex = pd[0] - pe[0];
    const coord_t aey = pa[1] - pe[1], bey = pb[1] - pe[1], cey = pc[1] - pe[1],
                  dey = pd[1] - pe[1];
    const coord_t aez = pa[2] - pe[2], bez = pb[2] - pe[2], cez = pc[2] - pe[2],
                  dez = pd[2] - pe[2];
  ...
  static inline bool innerSegmentsCross(const pointType &A, const pointType &B,
                                        const pointType &P,
                                        const pointType &Q) {
    int o11, o12, o21, o22;
