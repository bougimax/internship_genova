...
public:
  ...
  pointType *get_split_point(edge e);
  ...
  double
  get_energy_from_collapsing(edge e,
                             std::vector<tetrahedra> &incident_tetrahedras_v2);
  ...
  bool collapse_on_v1(edge e, std::vector<tetrahedra> &impacted_tetrahedras,
  ...
  void swap_edge_and_update(edge edge_to_swap, vertex collapse_vertex,
                            better_priority_queue::updatable_priority_queue<
                                size_t, std::pair<double, vertex>> &queue);

  void swap_edge(edge edge_to_swap, vertex collapse_vertex,
  ...
  // Return TRUE if the tetrahedra t is fully inside the ball centered on v
  // and of radius length MAX
  ...
  // Put in tets all the tetrahedras that are completely in the ball centered
  // on v of radius length MAX
