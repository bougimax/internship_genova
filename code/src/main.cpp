...
// createSteinerCDT
//
// 'plc' is a valid input PLC to the process. Validity is assumed but not
// verified! 'options' is a (possibly empty) string of characters, each
// controlling one option as follows: l: log results to cdt_log.csv b: add eight
// vertices to enclose everything in a box v: verbose mode f: try to make the
// output representable using floating point w: log to screen
...
TetMesh *createSteinerCDT(inputPLC &plc, const char *options) {
  ...
  if (verbose)
    printf("DT of the vertices built\n");
...
bool saveOutputFile(TetMesh &tin, const char *filename, const char *options) {
  bool rational = false, binary = false, erode = false, skin = false,
       medit = false;
  for (int i = 0; i < strlen(options); i++)
    switch (options[i]) {
    case 'q':
  ...
  if (medit) {
    if (rational || binary) {
      std::cerr << "Rational and binary modes are not available when saving to "
                   "MEDIT format\n";
      ret = false;
    } else {
    ...
    if (!rational && binary)
      ret &= tin.saveBinaryTET(tetfilename, erode);
    if (rational && !binary)
      ret &= tin.saveRationalTET(tetfilename, erode);
    if (rational && binary) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    ...
    std::cout
        << "-r: remove outer tetrahedra from output (if input is closed)\n";
    std::cout << "-s: saves skin to an ASCII OFF file (triangles between IN "
                 "and OUT)\n";
    std::cout << "OUTPUT:\n";
    std::cout << "Output has same name (and path) as input with an extension "
                 "appended.\n";
    std::cout << "E.g. CDT my_dir/test.off produces my_dir/test.off.tet\n";
    std::cout << "E.g. CDT -s my_dir/test.off produces my_dir/test.off.tet and "
                 "my_dir/test.off.off\n";
    return 0;
  }
