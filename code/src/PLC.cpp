...
bool PLCx::isAcute(const uint32_t vi,
  ...
  for (uint32_t i = 0; i < vv[vi].size(); i++)
    for (uint32_t j = 0; j < i; j++)
      if (isAcuteAngle(delmesh.vertices[vv[vi][i]], vip,
                       delmesh.vertices[vv[vi][j]]))
        return true;
...
void PLCx::edgeSplit(const uint32_t ei, pointType *Pt_c, uint32_t acute_v_id) {
  ...
  // 1-Create new vertex
  const uint32_t Pt_i =
      (uint32_t)delmesh.vertices.size(); // New vertex (mid point of e) index.
...
void PLCx::splitStrategy2(const uint32_t ei, const uint32_t ref) {
  ...
  if (vector3d::isCloserThan(vc_p, delmesh.vertices[e.ep[1]],
                             delmesh.vertices[ref])) {
    delete vc_p;
...
void PLCx::makePLCfaces() {
  ...
  // Init flat vertices
  for (size_t i = 0; i < delmesh.vertices.size(); i++)
    delmesh.marked_vertex[i] = 0;
  for (PLCface &f : faces) {
...
bool PLCx::lineIntersectsFace(uint32_t v1, uint32_t v2, const PLCface &f) {
  ...
  for (uint32_t i : f.triangles) {
    const uint32_t t[3] = {input_tv[i * 3], input_tv[i * 3 + 1],
                           input_tv[i * 3 + 2]};
    const pointType *tv[3] = {delmesh.vertices[t[0]], delmesh.vertices[t[1]],
                              delmesh.vertices[t[2]]};
    ...
    for (int j = i + 1; j < 4; j++)
      if (!delmesh.marked_vertex[n[i]] && !delmesh.marked_vertex[n[j]] &&
          ...
          pointType::lineCrossesTriangle(
              *delmesh.vertices[n[i]], *delmesh.vertices[n[j]],
              *delmesh.vertices[v1], *delmesh.vertices[v2],
              *delmesh.vertices[v3]))
        return true;
...
bool PLCx::adjacentFaceVertices(uint32_t v1, uint32_t v2, const PLCface &f) {
  if (!f.is_simply_connected || delmesh.marked_vertex[v1] > 1 ||
      delmesh.marked_vertex[v2] > 1) {
    for (PLCedge *e : f.bounding_edges)
...
void PLCx::getTetsIntersectingFace(uint32_t fi, std::vector<uint64_t> *i_tets,
  ...
  for (uint64_t t : et) {
    ...
    if (ov3 == 0) {
      ...
      if (!delmesh.marked_vertex[v_t[2]])
        continue;
    ...
    if (ov4 == 0) {
      ...
      if (!delmesh.marked_vertex[v_t[3]])
        continue;
  ...
  if (f.flat_vertices.size()) {
    ...
    for (uint64_t t : B)
      delmesh.mark_Tet_1(t);
  } else
    delmesh.mark_Tet_1(t0);
...
bool PLCx::isTriangleOnFace(
    ...
    const std::vector<std::pair<uint32_t, uint32_t>> &orig_flat_edges) {
  if (delmesh.marked_vertex[cv[0]] && delmesh.marked_vertex[cv[1]] &&
      delmesh.marked_vertex[cv[2]]) {
    const PLCface &f = faces[fi];
    ...
    if (f.flat_vertices.size()) {
      ...
      for (int i = 0; i < 3; i++) {
        uint32_t ev[2] = {cv[i], cv[(i + 1) % 3]};
        for (uint32_t v : f.flat_vertices)
          if (pointType::pointInInnerSegment(*delmesh.vertices[v],
                                             *delmesh.vertices[ev[0]],
                                             *delmesh.vertices[ev[1]]))
            return true;
      }
    }
  }
...
size_t PLCx::markInnerTets() {
  ...
  if (!is_polyhedron) {
    size_t ng = 0;
    for (size_t i = 0; i < delmesh.numTets(); i++)
      if (delmesh.isGhost(i))
        delmesh.mark_tetrahedra[i] = DT_OUT;
      else {
  ...
  // for (uint64_t t = 0; t < delmesh.tet_node.size(); t++)
  //     if (!delmesh.isGhost(t >> 2) && cornerMask[t] &&
  //         (delmesh.isGhost(delmesh.tet_neigh[t]>>2) || t >
  //         delmesh.tet_neigh[t])) {
  //     uint32_t v[3];
...
bool PLCx::recoverFaceHSi(std::vector<uint64_t> &i_tets, const PLCface &f,
  ...
  for (uint64_t &t : i_tets) {
    while (delmesh.is_marked_Tet_1(last))
      last--;
    if (t < last) {
  ...
  for (uint64_t t : i_tets) {
    ...
    for (int i = 0; i < 4; i++)
      if (!delmesh.is_marked_Tet_1(neigh[i] >> 2)) {
...
uint64_t PLCx::missingFaceInCavity(const std::vector<uint64_t> &bnd,
  ...
  for (uint32_t v : vertices)
    dt.vertices[i++] = delmesh.vertices[v];
...
uint64_t PLCx::meshCavity(const std::vector<uint64_t> &bnd,
  ...
  for (i = 0; i < dt.numTets(); i++)
    dt.mark_tetrahedra[i] = DT_UNKNOWN; // Might be redundant... CHECK
  ...
  for (i = 0; i < dt.tet_node.size(); i += 4)
    if (dt.tet_node[i + 3] == INFINITE_VERTEX &&
        (v_orient[vertices[dt.tet_node[i]]] ||
         v_orient[vertices[dt.tet_node[i + 1]]] ||
         v_orient[vertices[dt.tet_node[i + 2]]]))
      break;
  ...
  last = dt.numTets() - 1;
  while (dt.mark_tetrahedra[last] == DT_OUT)
    last--;
  for (uint64_t t = 0; t < last; t++)
  ...
  for (size_t i = 0; i < dt.numTets(); i++) {
    if (dt.mark_tetrahedra[i] == DT_OUT) {
      ...
      dt.inc_tet[dt.tet_node[i * 4 + 0]] = dt.inc_tet[dt.tet_node[i * 4 + 1]] =
          dt.inc_tet[dt.tet_node[i * 4 + 2]] =
              dt.inc_tet[dt.tet_node[i * 4 + 3]] = i;
    }
  }
  ...
  for (size_t i = 0; i < dt.tet_neigh.size(); i++)
    dt.tet_neigh[i] += delmesh.tet_neigh.size();
  ...
  for (size_t i = 0; i < dt.tet_node.size(); i++)
    if (dt.tet_node[i] != INFINITE_VERTEX)
      dt.tet_node[i] = vertices[dt.tet_node[i]];
  ...
  delmesh.tet_node.insert(delmesh.tet_node.end(), dt.tet_node.begin(),
                          dt.tet_node.end());
  delmesh.tet_neigh.insert(delmesh.tet_neigh.end(), dt.tet_neigh.begin(),
                           dt.tet_neigh.end());
  delmesh.mark_tetrahedra.resize(
      delmesh.mark_tetrahedra.size() + dt.mark_tetrahedra.size(), 0);
