...
double PLCx::getT1(uint32_t oe0i, uint32_t e0i) const {
    const std::vector<pointType *> &vs = delmesh.mesh->vertices;
    if (vs[e0i] == vs[oe0i])
        return 0.0;
    else if (&vs[e0i]->toLNC().P() == vs[oe0i])
        return vs[e0i]->toLNC().T();
    else
        return 1.0 - vs[e0i]->toLNC().T();
}
...
double PLCx::getT2(uint32_t oe1i, uint32_t e1i) const {
    ...
    if (vs[e1i] == vs[oe1i])
        return 1.0;
    else if (&vs[e1i]->toLNC().Q() == vs[oe1i])
        return vs[e1i]->toLNC().T();
    else
        return 1.0 - vs[e1i]->toLNC().T();
}

inline implicitPoint_LNC *
PLCx::getProjectionOrMidPoint(uint32_t oe0i, uint32_t oe1i, uint32_t e0i,
        uint32_t e1i, uint32_t ri,
        uint32_t &acute_v) const {
    const std::vector<pointType *> &vs = delmesh.mesh->vertices;
    ...
    if (discr <= (t1 + dv) || discr >= (t2 - dv)) {
        discr = (t1 + t2) / 2;
        acute_v = UINT32_MAX;
    } else
        acute_v = oe0i;

    return new implicitPoint_LNC(vs[oe0i]->toExplicit3D(),
            vs[oe1i]->toExplicit3D(), discr);
}

inline implicitPoint_LNC *
PLCx::getProjectionOrMidPoint_noac(uint32_t oe0i, uint32_t oe1i, uint32_t e0i,
        uint32_t e1i, uint32_t ri,
        uint32_t &acute_v) const {
    const std::vector<pointType *> &vs = delmesh.mesh->vertices;
    const vector3d e_ref(vs[ri]);
...
inline implicitPoint_LNC *
PLCx::getProjectionOrMidPoint_noac_rev(uint32_t oe0i, uint32_t oe1i,
        uint32_t e0i, uint32_t e1i, uint32_t ri,
        uint32_t &acute_v) const {
    const std::vector<pointType *> &vs = delmesh.mesh->vertices;
    const vector3d e_ref(vs[ri]);
...
inline implicitPoint_LNC *PLCx::getMidPoint(uint32_t oe0i, uint32_t oe1i,
        uint32_t e0i, uint32_t e1i) const {
    const std::vector<pointType *> &vs = delmesh.mesh->vertices;
    const double t1 = getT1(oe0i, e0i);
    const double t2 = getT2(oe1i, e1i);
    return new implicitPoint_LNC(vs[oe0i]->toExplicit3D(),
            vs[oe1i]->toExplicit3D(), (t1 + t2) / 2);
}
#else
implicitPoint_LNC *PLCx::getProjectionOrMidPoint(uint32_t oe0i, uint32_t oe1i,
        uint32_t e0i, uint32_t e1i,
        uint32_t ri,
        uint32_t &acute_v) const {
    const std::vector<pointType *> &vs = delmesh.mesh->vertices;
    const vector3d e_ref(vs[ri]);
...
implicitPoint_LNC *PLCx::getProjectionOrMidPoint_noac(uint32_t oe0i,
        uint32_t oe1i,
        uint32_t e0i,
        uint32_t e1i, uint32_t ri,
        uint32_t &acute_v) const {
...
implicitPoint_LNC *PLCx::getMidPoint(uint32_t oe0i, uint32_t oe1i, uint32_t e0i,
        uint32_t e1i) const {
    const std::vector<pointType *> &vs = delmesh.mesh->vertices;
    const vector3d e_e0(vs[e0i]);
    const vector3d e_e1(vs[e1i]);
...
// void saveTETS(const char* filename, std::vector<uint32_t>& newtets, TetMesh&
// tin)
//{
//     ofstream f(filename);
//
//     if (!f) ip_error("\nTetMesh::saveTET: FATAL ERROR cannot open the
//     file.\n");
//
...
void PLCface::zip() {
    ...
    while (!bounding_edges.empty() &&
            bounding_edges.front() == bounding_edges.back()) {
        std::rotate(bounding_edges.begin(), bounding_edges.begin() + 1,
                bounding_edges.end());
    ...
    do {
        ...
        for (size_t i = 1; i < bounding_edges.size(); i++) {
            ...
            if (e1 == e2) {
                bounding_edges.erase(bounding_edges.begin() + i - 1,
                        bounding_edges.begin() + i + 1);
                iterate = true;
            }
        }
    } while (iterate);
...
void PLCface::initConvexity(const PLCx &plc) {
    if (triangles.size() == 1)
        is_convex = true; // A single triangle is always convex
    else {
        // Calculate a valid 2D projection plane for the face.
        const uint32_t t0 = triangles[0];
        const uint32_t tv[3] = {plc.input_tv[t0 * 3], plc.input_tv[t0 * 3 + 1],
            plc.input_tv[t0 * 3 + 2]};
        const TetMeshTetrahedrizer &dm = plc.delmesh;
        const std::vector<pointType *> &dmv = dm.mesh->vertices;
        ...
        max_comp_normal = pointType::maxComponentInTriangleNormal(
                tp1.X(), tp1.Y(), tp1.Z(), tp2.X(), tp2.Y(), tp2.Z(), tp3.X(), tp3.Y(),
                tp3.Z());
        ...
        for (size_t i = 0; i < bounding_edges.size(); i++) {
            const PLCedge *e1 = bounding_edges[i];
            size_t i2;
            for (i2 = (i + 1) % bounding_edges.size(); i2 < bounding_edges.size();
                    i2++)
                if (!bounding_edges[i2 % bounding_edges.size()]->hasOriginalVertices(
                            e1->oep[0], e1->oep[1]))
                    break;
            ...
            const pointType *c1 = dm.mesh->vertices[v1];
            const pointType *c2 = dm.mesh->vertices[v2];
            const pointType *c3 = dm.mesh->vertices[v3];
            int co;
            if (max_comp_normal == 0)
                co = pointType::orient2Dyz(*c1, *c2, *c3);
            else if (max_comp_normal == 1)
                co = pointType::orient2Dzx(*c1, *c2, *c3);
            else
                co = pointType::orient2Dxy(*c1, *c2, *c3);
            if (co < 0)
                neg = true;
            if (co > 0)
                pos = true;
...
bool PLCx::faceHasTriangle(const PLCface &f, const uint32_t tv[3]) const {
    for (const uint32_t t : f.triangles) {
        ...
        if ((ftv[0] == tv[0] || ftv[0] == tv[1] || ftv[0] == tv[2]) &&
                ...
                (ftv[2] == tv[0] || ftv[2] == tv[1] || ftv[2] == tv[2]))
            return true;
    }
    return false;
}
...
void PLCx::makeVertexTriangleMaps(
        std::vector<std::vector<std::vector<uint32_t>>> &vt_maps) {
    vt_maps.clear();
    vt_maps.resize(faces.size());

    v_reindex.resize(delmesh.mesh->vertices.size(), UINT32_MAX);
    std::vector<bool> orig_tri_mark(input_nt, false);
    for (size_t i = 0; i < faces.size(); i++)
        makeVertexTriangleMap(faces[i], vt_maps[i], orig_tri_mark);
}

void PLCx::makeVertexTriangleMap(PLCface &f,
        std::vector<std::vector<uint32_t>> &vt_map,
    ...
    for (size_t vi = 0; vi < f.vertices.size(); vi++)
        if (isSteinerVertex(f.vertices[vi])) {
            assert(delmesh.mesh->marked_vertex[f.vertices[vi]] == 1);
    ...
    for (uint32_t i : f.triangles)
        for (uint32_t j = 0; j < 3; j++) {
            ...
            if (v_reindex[v] != UINT32_MAX) {
                if (delmesh.mesh->marked_vertex[v] == 1)
                    vt_map[v_reindex[v]].push_back(i);
                else {
                    for (auto p : singular_v)
                        if (p.first == v)
                            vt_map[p.second].push_back(i);
                }
            }
        }

    for (uint32_t i : f.triangles)
        orig_tri_mark[i] = false;
...
void PLCx::mergePreEdges() {
    ...
    for (uint32_t ei = 0; ei < edges.size() - 1;) {
        ...
        while (++ei < edges.size() && e.ep[0] == edges[ei].ep[0] &&
                e.ep[1] == edges[ei].ep[1]) {
            PLCedge &ne = edges[ei];
            // each edge initially has only one incident triangle, ne never been
            // merged
    ...
    edges.erase(
            std::remove_if(edges.begin(), edges.end(), PLCedge::isIsolatedPtr),
            edges.end());
}
...
// edges is positive.
bool PLCx::isAcute(const uint32_t vi,
        const std::vector<std::vector<uint32_t>> &vv) const {
    const pointType *vip = delmesh.mesh->vertices[vi];
    for (uint32_t i = 0; i < vv[vi].size(); i++)
        for (uint32_t j = 0; j < i; j++)
            if (isAcuteAngle(delmesh.mesh->vertices[vv[vi][i]], vip,
                        delmesh.mesh->vertices[vv[vi][j]]))
                return true;
...
void PLCx::initialize() {
    std::vector<bool> is_acute(delmesh.mesh->numVertices(), false);
    ...
    vv.resize(delmesh.mesh->vertices.size());
    ...
    // Mark acute vertices
    is_acute.resize(delmesh.mesh->vertices.size());
    for (uint32_t vi = 0; vi < delmesh.mesh->vertices.size(); vi++)
        is_acute[vi] = isAcute(vi, vv);

    // Classify non-flat edges by type
    for (uint32_t ei = 0; ei < edges.size(); ei++)
        if (edges[ei].type != flat) {
            PLCedge &e = edges[ei];
            const bool e0_acute = is_acute[e.ep[0]];
            const bool e1_acute = is_acute[e.ep[1]];
            if (e0_acute && e1_acute)
                e.type = both_acute_ep;
    ...
    delmesh.mesh->marked_vertex.resize(delmesh.mesh->vertices.size());
}
...
bool PLCx::is_missing_PLCedge(const uint32_t ei) const {
    return !delmesh.mesh->hasEdge(edges[ei].ep[0], edges[ei].ep[1]);
}
...
void PLCx::find_missing_PLCedges(std::vector<uint32_t> &me) const {
    for (uint32_t ei = 0; ei < edges.size(); ei++) {
        ...
        if (e.type != flat && is_missing_PLCedge(ei))
            me.push_back(ei);
    }
}

// Splits PLCedge e, inserting the point Pt (which is internal to the edge),
// inserts Pt in the vector vertices of TetMesh DS (do not update
// tetrahedrization), updates PLCx DS. This split function works only for edges
// of types: "no_acute_ep", "one_acute_ep"
void PLCx::edgeSplit(const uint32_t ei, pointType *Pt_c, uint32_t acute_v_id) {
    ...
    const uint32_t Pt_i =
        (uint32_t)
        delmesh.mesh->vertices.size(); // New vertex (mid point of e) index.
...
// inserts Mpt in the vector vertices of TetMesh DS (do not update
// tetrahedrization), updates PLCx DS. This split function works only for edges
// of types: "no_acute_ep", "one_acute_ep", "both_acute_ep"
void PLCx::middleEdgeSplit(const uint32_t ei) {
    PLCedge &e = edges[ei];
    const uint32_t e0 = e.ep[0], e1 = e.ep[1]; // Memory partent edge endpoints
    const uint32_t oe0 = e.oep[0],
    oe1 = e.oep[1]; // Memory partent edge endpoints

    implicitPoint_LNC *np = getMidPoint(oe0, oe1, e0, e1);

    // 1-Create new vertex
    const uint32_t Mpt_i =
        (uint32_t)
        delmesh.mesh->vertices.size(); // New vertex (mid point of e) index.
    ...
    if (e.type == both_acute_ep) {
        e.type = one_acute_ep;
        edges.push_back(
                PLCedge(e1, Mpt_i, e.oep[1], e.oep[0], e.inc_tri, one_acute_ep));
    } else
        edges.push_back(PLCedge(Mpt_i, e1, e.oep[0], e.oep[1], e.inc_tri, e.type));
}
...
void PLCx::splitStrategy1(const uint32_t ei, const uint32_t ref) {
    ...
    const pointType *e0p = delmesh.mesh->vertices[e.ep[0]];
    const pointType *e1p = delmesh.mesh->vertices[e.ep[1]];
    const pointType *refp = delmesh.mesh->vertices[ref];
    implicitPoint_LNC *vc_p;

    uint32_t acute_v = UINT32_MAX;

    if (vector3d::isAtMostTwiceDistanceThan(e0p, refp, e1p))
        vc_p = getProjectionOrMidPoint_noac(e.oep[0], e.oep[1], e.ep[0], e.ep[1],
                ref, acute_v);
    else if (vector3d::isAtMostTwiceDistanceThan(e1p, refp, e0p))
        vc_p = getProjectionOrMidPoint_noac_rev(e.oep[0], e.oep[1], e.ep[0],
                e.ep[1], ref, acute_v);
    else
...
void PLCx::splitStrategy2(const uint32_t ei, const uint32_t ref) {
    ...
    uint32_t acute_v;
    vc_p = getProjectionOrMidPoint(e.oep[0], e.oep[1], e.ep[0], e.ep[1], ref,
            acute_v);

    if (vector3d::isCloserThan(vc_p, delmesh.mesh->vertices[e.ep[1]],
                delmesh.mesh->vertices[ref])) {
        delete vc_p;
...
bool PLCx::splitMissingEdge(uint32_t mei) {
    ...
    if (e.type == both_acute_ep) {
        ct = delmesh.mesh->inc_tet[e.ep[0]] << 2;
        middleEdgeSplit(mei);
    } else {
        const uint32_t ref = delmesh.findEncroachingPoint(e.ep[0], e.ep[1], ct);
        if (ref == UINT32_MAX) { // This should never happen
            printf("WARNING: Could not find a valid encroaching point! Using "
                    "midpoint...\n");
            ct = delmesh.mesh->inc_tet[e.ep[0]] << 2;
            middleEdgeSplit(mei);
        } else if (e.type == no_acute_ep)
            splitStrategy1(mei, ref);
        else
            splitStrategy2(mei, ref);
    }

    // 2-UPDATE TetMesh by tetrahedralizing around steiner_point (last elem of
    // vertices)
    delmesh.insertExistingVertex((uint32_t)delmesh.mesh->vertices.size() - 1, ct);
...
void PLCx::segmentRecovery_HSi(bool quiet) {
    for (uint64_t tet_i = 0; tet_i < delmesh.mesh->numTets(); tet_i++)
        delmesh.mesh->unmark_Tet_1(tet_i);
    ...
    std::vector<bool> touched_vertex(delmesh.mesh->vertices.size(), false);
    ...
    while (!miss_edges.empty()) {
        ...
        while (!miss_edges.empty()) {
            ...
            const uint32_t vm = delmesh.mesh->numVertices();
            miss_edges.pop_back();
            ...
            if (splitMissingEdge(ei)) {
                ...
                delmesh.mesh->VV(vm, loval_vv);
                for (uint32_t w : loval_vv)
                    touched_vertex[w] = true;
                touched_vertex.push_back(true);
                loval_vv.clear();

                num_st++;
                if (!quiet && (num_st & 1023) == 1023) {
                    printf("\rMissing edges: %zu (Tot %zu Steiner points inserted)       "
                            "          ",
                            nme, num_st);
                    fflush(stdout);
                }
        ...
        for (uint32_t ei = 0; ei < edges.size(); ei++) {
            const PLCedge &e = edges[ei];
            if (e.type != flat &&
                    (touched_vertex[e.ep[0]] && touched_vertex[e.ep[1]]) &&
                    is_missing_PLCedge(ei))
                miss_edges.push_back(ei);
        }
        for (uint32_t i = 0; i < delmesh.mesh->numVertices(); i++)
            touched_vertex[i] = 0;
...
class iEdge {
    public:
        ...
        iEdge(uint32_t _v1, uint32_t _v2, uint32_t t) : v1(_v1), v2(_v2), iTris{t} {
            if (v1 > v2)
                std::swap(v1, v2);
        }
        bool operator<(const iEdge &e) const {
            return (2 * ((v1 < e.v1) - (v1 > e.v1)) + ((v2 < e.v2) - (v2 > e.v2))) > 0;
        }
        bool operator==(const iEdge &e) const { return v1 == e.v1 && v2 == e.v2; }
        void absorb(iEdge &e) {
            iTris.insert(iTris.end(), e.iTris.begin(), e.iTris.end());
            e.iTris.clear();
        }
        ...
        void fillEdges(const std::vector<uint32_t> &te, std::vector<PLCedge> &edges) {
            for (size_t i = 0; i < te.size(); i++) {
                ...
                if (e.oep[0] == v2 && e.oep[1] == v1)
                    e.swap();
                if (e.oep[0] == v1 && e.oep[1] == v2)
                    pEdges.push_back(te[i]);
            }

            // This is a dumb O(n^2) sorting, but should be good enough for small
            // vectors
            uint32_t pv = v1;
        ...
        void copyOrderedEdges(PLCface &f, uint32_t tv1, uint32_t tv2, uint32_t tv3,
                std::vector<PLCedge> &edges) const {
            std::vector<PLCedge *> tie;
            tie.resize(pEdges.size());
            for (size_t i = 0; i < pEdges.size(); i++)
                tie[i] = &edges[pEdges[i]];

            if (tv1 == v1 && tv2 == v2) { // Insert at the beginning in same order
                f.bounding_edges.insert(f.bounding_edges.begin(), tie.begin(), tie.end());
            } else if (tv1 == v2 &&
                    tv2 == v1) { // Insert at the beginning in reverse order
                ...
                for (; pos < f.bounding_edges.size(); pos++) {
                    ...
                    if (e->ep[0] == tv3 || e->ep[1] == tv3)
                        break;
                }
                f.bounding_edges.insert(f.bounding_edges.begin() + pos, tie.begin(),
                        tie.end());
            } else if (tv2 == v2 &&
                    tv3 == v1) { // Insert in the middle in reverse order
                size_t pos = 0;
                for (; pos < f.bounding_edges.size(); pos++) {
                    const PLCedge *e = f.bounding_edges[pos];
                    if (e->ep[0] == tv3 || e->ep[1] == tv3)
...
void PLCface::absorb(PLCface &f, PLCedge *e) {
    ...
    pos = std::find(bounding_edges.begin(), bounding_edges.end(), e) -
        bounding_edges.begin();
    if (pos == bounding_edges.size())
        return;

    if (&f == this) {
        pos2 = pos + 1; // Make sure to get the other instance
        while (be[pos2] != e)
            pos2++; // e's position in ne
        ...
        if (pos)
            nb.insert(nb.end(), be.begin(), be.begin() + pos);
        if (pos2 != (ne.size() - 1))
            nb.insert(nb.end(), ne.begin() + pos2 + 1, ne.end());
        if (pos2)
            nb.insert(nb.end(), ne.begin(), ne.begin() + pos2);
        if (pos != (be.size() - 1))
            nb.insert(nb.end(), be.begin() + pos + 1, be.end());
...
void PLCx::makePLCfaces() {
    if (!faces.empty())
        return; // Faces were previously built
    ...
    for (size_t i = 1; i < iEdges.size(); i++)
        if (iEdges[i] == *prev)
            prev->absorb(iEdges[i]);
        else
            prev = &iEdges[i];
    iEdges.erase(std::remove_if(iEdges.begin(), iEdges.end(), iEdge::isEmpty),
            iEdges.end());

    // for each plcEdge e, for each inc_tri, associate e to inc_tri in a
    // map-vector triEdges
    std::vector<std::vector<uint32_t>> triEdges;
    triEdges.resize(input_nt);
    for (size_t i = 0; i < edges.size(); i++)
        for (uint32_t &t : edges[i].inc_tri)
            triEdges[t].push_back((uint32_t)i);

    // for each iEdge e, consider first iTri and copy submap from triEdges (only
    // edges that have v1-v2 as orig endpoints) sort pEdges consistently with
    // v1-v2
    for (iEdge &e : iEdges)
        e.fillEdges(triEdges[e.iTris[0]], edges);

    // Create a face for each input triangle
    faces.resize(input_nt);
    for (uint32_t i = 0; i < input_nt; i++)
        faces[i].triangles.push_back(i);

    // For each iEdge e, copy its ordered edge chain to each of its incident tris
    // in faces
    for (iEdge &e : iEdges)
        for (uint32_t t : e.iTris)
            e.copyOrderedEdges(faces[t], input_tv[t * 3], input_tv[t * 3 + 1],
                    input_tv[t * 3 + 2], edges);
    ...
    for (iEdge &e : iEdges)
        if (edges[e.pEdges.front()].type == flat) {
    ...
    for (iEdge &e : iEdges)
        if (edges[e.pEdges.front()].type == flat) {
            ...
            while (remap[t2i] != t2i)
                t2i = remap[t2i];
            if (t1i == t2i && !PLCface::isEmpty(faces[t1i]))
                faces[t1i].absorb(faces[t2i], &edges[e.pEdges[0]]);
        }

    faces.erase(std::remove_if(faces.begin(), faces.end(), PLCface::isEmpty),
            faces.end());
    ...
    // Init flat vertices
    for (size_t i = 0; i < delmesh.mesh->vertices.size(); i++)
        delmesh.mesh->marked_vertex[i] = 0;
    for (PLCface &f : faces) {
        for (PLCedge *e : f.bounding_edges)
            delmesh.mesh->marked_vertex[e->oep[0]] =
                delmesh.mesh->marked_vertex[e->oep[1]] = 1;

        for (uint32_t t : f.triangles)
            for (int i = 0; i < 3; i++) {
                const uint32_t v = input_tv[t * 3 + i];
                if (!delmesh.mesh->marked_vertex[v]) {
                    delmesh.mesh->marked_vertex[v] = 1;
                    f.flat_vertices.push_back(v);
                }
            }
        for (uint32_t t : f.triangles)
            for (int i = 0; i < 3; i++)
                delmesh.mesh->marked_vertex[input_tv[t * 3 + i]] = 0;
    }

    for (PLCface &f : faces)
        f.initConvexity(*this);
    for (PLCface &f : faces)
        initFaceFlatEdges(f);

    // For each face, for each of its vertices, set of incident face triangles
    makeVertexTriangleMaps(vt_maps);
    v_reindex.resize(delmesh.mesh->vertices.size());
}
...
bool PLCx::edgeIntersectsFace(uint32_t v1, uint32_t v2, const PLCface &f) {
    ...
    if ((o1 > 0 && o2 > 0) || (o1 < 0 && o2 < 0))
        ...
        return lineIntersectsFace(v1, v2, f);
}
...
bool PLCx::lineIntersectsFace(uint32_t v1, uint32_t v2, const PLCface &f) {
    const pointType *vp[2] = {delmesh.mesh->vertices[v1],
        delmesh.mesh->vertices[v2]};
    for (uint32_t i : f.triangles) {
        const uint32_t t[3] = {input_tv[i * 3], input_tv[i * 3 + 1],
            input_tv[i * 3 + 2]};
        const pointType *tv[3] = {delmesh.mesh->vertices[t[0]],
            delmesh.mesh->vertices[t[1]],
            delmesh.mesh->vertices[t[2]]};
        if (pointType::lineCrossesTriangle(*vp[0], *vp[1], *tv[0], *tv[1], *tv[2]))
            return true;
    }
...
bool PLCx::triangleIntersectsFace(uint64_t t, const PLCface &f) {
    ...
    for (int j = 0; tb < tb + 4; tb++)
        if (tb != t)
            tv[j++] = delmesh.mesh->tet_node[tb];

    return (edgeIntersectsFace(tv[0], tv[1], f) ||
            edgeIntersectsFace(tv[1], tv[2], f) ||
            edgeIntersectsFace(tv[2], tv[0], f));
}

bool PLCx::tetIntersectsFace(uint64_t t, const PLCface &f) {
    const uint32_t *n = delmesh.mesh->tet_node.data() + (t << 2);
    if (n[3] == INFINITE_VERTEX)
        return false;

    for (int i = 0; i < 4; i++)
        for (int j = i + 1; j < 4; j++)
            if (!delmesh.mesh->marked_vertex[n[i]] &&
                    !delmesh.mesh->marked_vertex[n[j]] &&
                    innerEdgeIntersectsFace(n[i], n[j], f))
                return true;

    return false;
}

bool PLCx::tetIntersectsInnerTriangle(uint64_t t, uint32_t v1, uint32_t v2,
        uint32_t v3) {
    const uint32_t *n = delmesh.mesh->tet_node.data() + (t << 2);
    if (n[3] == INFINITE_VERTEX)
        return false;
    ...
    for (int i = 0; i < 4; i++)
        for (int j = i + 1; j < 4; j++)
            if (!delmesh.mesh->marked_vertex[n[i]] &&
                    !delmesh.mesh->marked_vertex[n[j]] &&
                    (cachedOrient3D(n[i], v1, v2, v3) *
                     cachedOrient3D(n[j], v1, v2, v3)) <= 0 &&
                    pointType::lineCrossesTriangle(
                        *delmesh.mesh->vertices[n[i]], *delmesh.mesh->vertices[n[j]],
                        *delmesh.mesh->vertices[v1], *delmesh.mesh->vertices[v2],
                        *delmesh.mesh->vertices[v3]))
                return true;
...
bool PLCx::adjacentFaceVertices(uint32_t v1, uint32_t v2, const PLCface &f) {
    if (!f.is_simply_connected || delmesh.mesh->marked_vertex[v1] > 1 ||
            delmesh.mesh->marked_vertex[v2] > 1) {
        for (PLCedge *e : f.bounding_edges)
            if ((e->ep[0] == v1 && e->ep[1] == v2) ||
                    (e->ep[0] == v2 && e->ep[1] == v1))
                return true;
        return false;
    }
...
int PLCx::localOrient3d(uint32_t v1, uint32_t v2, uint32_t v3, uint32_t v4,
        std::vector<uint32_t> &to_unorient) {
    if (v_orient[v1] != UNDET_ORIENTATION)
        return v_orient[v1];
...
inline void pushAndMark(uint64_t t, TetMeshTetrahedrizer &m,
...
void PLCx::getTetsIntersectingFace(uint32_t fi, std::vector<uint64_t> *i_tets,
    ...
    delmesh.mesh->ET(v_t[0], v_t[1], et);

    const uint32_t *tv =
        input_tv +
        f.triangles[0] * 3; // Vertices of one face triangle for orientation

    // If face has only three vertices, just check whether they are in one tet of
    // the ET
    if (f.vertices.size() == 3 && f.flat_vertices.empty()) {
        const uint32_t ov =
            (tv[0] != v_t[0] && tv[0] != v_t[1])
            ? (tv[0])
            : ((tv[1] != v_t[0] && tv[1] != v_t[1]) ? (tv[1]) : (tv[2]));
        for (uint64_t t : et)
            if (delmesh.mesh->tetHasVertex(t, ov)) {
                if (cornerMask) {
                    const uint64_t c =
                        delmesh.mesh->tetOppositeCorner(t, v_t[0], v_t[1], ov);
                    (*cornerMask)[c] = (*cornerMask)[delmesh.mesh->tet_neigh[c]] = true;
                }
                return;
            }
    }
    ...
    const std::vector<std::pair<uint32_t, uint32_t>> &orig_flat_edges =
        f.orig_flat_edges;
    ...
    for (uint64_t t : et) {
        ...
        delmesh.mesh->oppositeTetEdge(t << 2, v_t, v_t + 2);

        // If we are using this function to mark faces we need three vertices on the
        // plane
        if (cornerMask) {
            if (v_orient[v_t[2]] == 0 && isTriangleOnFace(v_t, fi, orig_flat_edges)) {
                t0 = t;
                break;
            }
        ...
        //    - if v_orient[v3]!=0 e v_orient[v3]==v_orient[v4] -> skip (no
        //    intersection)
        ...
        // const uint32_t* n = delmesh.mesh->tet_node.data() + tb;

        //    - if v_orient[v3]*v_orient[v4]<0 e lineIntersectsFace(v3,v4,f) ->
        //    intersection
        ...
        if (ov3 == 0) {
            ...
            if (!delmesh.mesh->marked_vertex[v_t[2]])
                continue;
            //      - if v1,v2,v3 part of f
            if (isTriangleOnFace(v_t, fi, orig_flat_edges)) {
                t0 = t;
                break;
            }
        }
        ...
        if (ov4 == 0) {
            ...
            if (!delmesh.mesh->marked_vertex[v_t[3]])
                continue;
    ...
    if (f.flat_vertices.size()) {
        ...
        for (uint64_t t : B)
            delmesh.mesh->mark_Tet_1(t);
    } else
        delmesh.mesh->mark_Tet_1(t0);
    ...
    for (size_t k = 0; k < B.size(); k++) {
        ...
        const uint64_t tb = t << 2;
        const uint64_t *nn = delmesh.mesh->tet_neigh.data() + tb;
        const uint32_t *tn = delmesh.mesh->tet_node.data() + tb;
        // for each of the four neighbors n of t
        for (int i = 0; i < 4; i++) {
            const uint64_t n = nn[i] >> 2;
            //   if n is not a ghost and is not marked
            if (!delmesh.mesh->isGhost(n) && !delmesh.mesh->is_marked_Tet_1(n)) {
                //      let cv be the three common vertices of t and n
                const uint32_t cv[3] = {tn[(i + 1) & 3], tn[(i + 2) & 3],
                    tn[(i + 3) & 3]};
                const unsigned char mv[3] = {delmesh.mesh->marked_vertex[cv[0]],
                    delmesh.mesh->marked_vertex[cv[1]],
                    delmesh.mesh->marked_vertex[cv[2]]};
                const int o3d[3] = {
                ...
                if (mv[0] && mv[1] && mv[2])
                    pushAndMark(n, delmesh, B);
                else if (mv[0] && mv[1]) {
                    if (!adjacentFaceVertices(cv[0], cv[1], f))
                        pushAndMark(n, delmesh, B);
                } else if (mv[1] && mv[2]) {
                    if (!adjacentFaceVertices(cv[1], cv[2], f))
                        pushAndMark(n, delmesh, B);
                } else if (mv[2] && mv[0]) {
                    if (!adjacentFaceVertices(cv[2], cv[0], f))
                        pushAndMark(n, delmesh, B);
                } else if (mv[0]) {
                    if (o3d[1] * o3d[2] < 0)
                        pushAndMark(n, delmesh, B);
    ...
    ////for (uint32_t v : f.vertices) delmesh.mesh->marked_vertex[v] = 1;
    ////for (size_t i = 0; i < v_orient.size(); i++) v_orient[i] =
    /// UNDET_ORIENTATION;

    ////for (size_t i = 0; i < delmesh.mesh->numTets(); i++)
    ////    if (tetIntersectsFace(i, f))
    ////        i_tets.push_back(i);

    ////for (size_t i = 0; i < v_orient.size(); i++) v_orient[i] =
    /// UNDET_ORIENTATION; /for (uint32_t v : f.vertices)
    /// delmesh.mesh->marked_vertex[v]
    ///= 0; /return;
}

bool PLCx::segmentCrossesFlatEdge(
        uint32_t ev[2],
        const std::vector<std::pair<uint32_t, uint32_t>> &flat_edges,
        int max_comp_normal) {
    const pointType &A = *delmesh.mesh->vertices[ev[0]];
    const pointType &B = *delmesh.mesh->vertices[ev[1]];
    for (auto &e : flat_edges) {
        if ((e.first == ev[0] && e.second == ev[1]) ||
                (e.first == ev[1] && e.second == ev[0]))
            return false;
        if (e.first == ev[0] || e.second == ev[1] || e.first == ev[1] ||
                e.second == ev[0])
            continue;
        const pointType &P = *delmesh.mesh->vertices[e.first];
        const pointType &Q = *delmesh.mesh->vertices[e.second];

        if (pointType::innerSegmentsCross(A, B, P, Q, max_comp_normal))
            return true;
    }
    return false;
}
...
bool PLCx::isTriangleOnFace(
        const uint32_t cv[3], uint32_t fi,
        const std::vector<std::pair<uint32_t, uint32_t>> &orig_flat_edges) {
    if (delmesh.mesh->marked_vertex[cv[0]] &&
            delmesh.mesh->marked_vertex[cv[1]] &&
            delmesh.mesh->marked_vertex[cv[2]]) {
        const PLCface &f = faces[fi];
        if (f.is_convex || faceHasTriangle(f, cv))
            return true;
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (segmentCrossesFlatEdge(ev, orig_flat_edges, f.max_comp_normal))
                return true;
        }

        if (f.flat_vertices.size()) {
            for (int i = 0; i < 3; i++)
                if (std::find(f.flat_vertices.begin(), f.flat_vertices.end(), cv[i]) !=
                        f.flat_vertices.end())
                    return true;
            ...
            for (int i = 0; i < 3; i++) {
                uint32_t ev[2] = {cv[i], cv[(i + 1) % 3]};
                for (uint32_t v : f.flat_vertices)
                    if (pointType::pointInInnerSegment(*delmesh.mesh->vertices[v],
                                *delmesh.mesh->vertices[ev[0]],
                                *delmesh.mesh->vertices[ev[1]]))
                        return true;
            }
        }
    }
...
void PLCx::initFaceFlatEdges(PLCface &f) {
    ...
    for (uint32_t t : f.triangles) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (ev[1] < ev[0])
                std::swap(ev[0], ev[1]);
            if (!orig_edges.insert(std::pair<uint32_t, uint32_t>(ev[0], ev[1]))
                    .second)
                f.orig_flat_edges.push_back(
                        std::pair<uint32_t, uint32_t>(ev[0], ev[1]));
        }
    }
}
...
size_t PLCx::markInnerTets() {
    ...
    // If the PLC does not define a valid polyhedron, just tag every tet as IN but
    // the ghosts
    if (!is_polyhedron) {
        size_t ng = 0;
        for (size_t i = 0; i < delmesh.mesh->numTets(); i++)
            if (delmesh.mesh->isGhost(i))
                delmesh.mesh->mark_tetrahedra[i] = DT_OUT;
            else {
                delmesh.mesh->mark_tetrahedra[i] = DT_IN;
                ng++;
            }
        return ng;
    }
    ...
    std::vector<bool> cornerMask(delmesh.mesh->tet_node.size(), false);

    // Crea relazione VF
    //   per ogni faccia f aggiungi f alla VF di tutti i suoi bounding e internal
    //   vertices
    // Per ogni triangolo in delmesh, cerca la(le) faccia comune f in VF(v1),
    // VF(v2) e VF(v3)
    //   se c'è più di una faccia comune marca immediatamente il triangolo e passa
    //   oltre altrimenti
    // scopri se il triangolo sta dentro o fuori dalla faccia comune f (orient2d?)
    //   1) se f è convessa
    //   2) se v1 è interno a f (o v2, o v3)
    //   3) se il baricentro del triangolo è interno a uno dei triangoli di f e al
    //   triangolo stesso (check per possibile errore numerico)
    // Se la faccia comune esiste e il triangolo ci sta dentro allora marcala,
    // altrimenti no
    ...
    // for (uint64_t t = 0; t < delmesh.mesh->tet_node.size(); t++)
    //    if (!delmesh.mesh->isGhost(t>>2) && cornerMask[t] &&
    //        (delmesh.mesh->isGhost(delmesh.mesh->tet_neigh[t]>>2) ||
    //        t>delmesh.mesh->tet_neigh[t])) num_constrained_faces++;

    // ofstream f("constraints.off");
    // if (!f) ip_error("PLCx::savePLC: Cannot open file.\n");

    // f << "OFF\n";
    // f << delmesh.mesh->numVertices() << " " << num_constrained_faces << " 0\n";

    // for (uint32_t i = 0; i < delmesh.mesh->numVertices(); i++)
    //     f << *delmesh.mesh->vertices[i] << "\n";

    // for (uint64_t t = 0; t < delmesh.mesh->tet_node.size(); t++)
    //     if (!delmesh.mesh->isGhost(t >> 2) && cornerMask[t] &&
    //         (delmesh.mesh->isGhost(delmesh.mesh->tet_neigh[t]>>2) || t >
    //         delmesh.mesh->tet_neigh[t])) {
    //     uint32_t v[3];
    //     delmesh.mesh->getFaceVertices(t, v);
    //     f << "3 " << v[0] << " " << v[1] << " " << v[2] << "\n";
    // }
...
bool PLCx::faceRecovery(bool quiet) {
    ...
    v_orient.resize(delmesh.mesh->vertices.size(), UNDET_ORIENTATION);

    delmesh.mesh->marked_vertex.assign(delmesh.mesh->vertices.size(), 0);
    for (uint64_t tet_i = 0; tet_i < delmesh.mesh->numTets(); tet_i++)
        delmesh.mesh->unmark_Tet_1(tet_i);
    ...
    do {
        ...
        for (size_t i = 0; i < faces.size(); i++) {
            ...
            const uint32_t *tv =
                input_tv + f.triangles[0] * 3; // The vertices of f for orientation
            std::vector<uint64_t> i_tets;
            getTetsIntersectingFace((uint32_t)i, &i_tets);

            if (i_tets.size()) {
                if (!quiet)
                    printf("\rRecovering face: %zu                  ", i);
                fflush(stdout);
                std::vector<uint32_t> cavity_v;
                for (uint32_t v : f.vertices)
                    v_orient[v] = 0;
                for (uint64_t t : i_tets) {
                    const uint32_t *v = delmesh.mesh->getTetNodes(t << 2);
                    for (int j = 0; j < 4; j++)
                        if (v_orient[v[j]] == UNDET_ORIENTATION) {
                            v_orient[v[j]] =
                                delmesh.vOrient3D(v[j], tv[0], tv[1], tv[2]);
                ...
                if (!recoverFaceHSi(i_tets, f, sisMethodWorks))
                    needRecursion = true;

                i_tets.clear();

                // Reset orientations for the future
                for (uint32_t v : cavity_v)
                    v_orient[v] = UNDET_ORIENTATION;
                for (uint32_t v : f.vertices)
                    v_orient[v] = UNDET_ORIENTATION;
            }
...
bool PLCx::recoverFaceHSi(std::vector<uint64_t> &i_tets, const PLCface &f,
        bool &sisMethodWorks) {

    // std::vector<uint32_t> oldtets;
    // for (uint64_t t : i_tets) {
    //     const uint32_t* tn = delmesh.mesh->tet_node.data() + (t * 4);
    ...
    uint32_t last = delmesh.mesh->numTets() - 1;
    for (uint64_t &t : i_tets) {
        while (delmesh.mesh->is_marked_Tet_1(last))
            last--;
        if (t < last) {
            delmesh.mesh->swapTets(t, last);
            t = last;
        }
    }
    while (delmesh.mesh->is_marked_Tet_1(last))
    ...
    for (uint64_t t : i_tets) {
        const uint64_t *neigh = delmesh.mesh->getTetNeighs(t << 2);
        for (int i = 0; i < 4; i++)
            if (!delmesh.mesh->is_marked_Tet_1(neigh[i] >> 2)) {
                if (isUpperCavityTet(neigh[i]))
    ...
    for (uint64_t t : i_tets) {
        for (int i = 0; i < 4; i++) {
            const uint32_t v = delmesh.mesh->tet_node[t * 4 + i];
            if (!delmesh.mesh->marked_vertex[v]) {
                delmesh.mesh->marked_vertex[v] = 1;
                if (v_orient[v] >= 0)
    ...
    for (uint32_t w : f.flat_vertices)
        if (!delmesh.mesh->marked_vertex[w]) {
            delmesh.mesh->marked_vertex[w] = 1;
    ...
    for (uint32_t w : top_vertices)
        delmesh.mesh->marked_vertex[w] = 0;
    ...
    while ((t = missingFaceInCavity(top_faces, top_vertices)) != UINT64_MAX) {
        ...
        if (v_orient[top_vertices.back()] <
                0) { // This might happen due to a theoretical bug in Si's paper
            cavity_ok = false;
            break;
        }
        std::sort(top_vertices.begin(), top_vertices.end());
    }
    while ((t = missingFaceInCavity(bottom_faces, bottom_vertices)) !=
            UINT64_MAX) {
        toremove.push_back(expandCavity(bottom_faces, bottom_vertices, t, f));
        if (v_orient[bottom_vertices.back()] >
                0) { // This might happen due to a theoretical bug in Si's paper
            cavity_ok = false;
            break;
        }
    ...
    if (cavity_ok) {
        delmesh.mesh->tet_node.resize(last * 4);
        delmesh.mesh->tet_neigh.resize(last * 4);
        delmesh.mesh->mark_tetrahedra.resize(last);
    ...
    if (!toremove.empty()) {
        if (cavity_ok) {
            for (uint32_t v : top_vertices)
                v_orient[v] = UNDET_ORIENTATION;
            for (uint32_t v : bottom_vertices)
                v_orient[v] = UNDET_ORIENTATION;
            for (uint64_t y : toremove)
                delmesh.pushAndMarkDeletedTets(y);
            delmesh.removeDelTets();
            return false;
        }
        // If Hang Si's cavity expansion fails revert to slower gift-wrap method
            ...
            if (!f.flat_vertices.empty())
                ip_error("Hang Si's cavity expansion fails on a flat face with "
                        "internal vertices.\nTreatment of this very particular case "
                        "was not implemented!\n");
            delmesh.recoverFaceGiftWrap(i_tets, v_orient);
            sisMethodWorks = false;
            for (uint32_t v : top_vertices)
...
uint64_t PLCx::expandCavity(std::vector<uint64_t> &bnd,
        std::vector<uint32_t> &vertices, uint64_t t,
        const PLCface &f) {
    assert(!delmesh.mesh->isGhost(t >> 2));
    ...
    for (uint64_t i = 0; i < 4; i++) {
        ...
        if (y != t) {
            ...
            if (pos != bnd.end())
                ...
                bnd.push_back(delmesh.mesh->tet_neigh[y]);
                added++;
            }
        }
    }
    ...
    if (added == 3) {
        const uint32_t v = delmesh.mesh->tet_node[t];
...
class bdUpdater {
    public:
        ...
        bdUpdater(uint64_t _t1, uint64_t _t2, uint64_t _bnd)
            : t1(_t1), t2(_t2), bnd(_bnd) {}
};

uint64_t PLCx::missingFaceInCavity(const std::vector<uint64_t> &bnd,
        const std::vector<uint32_t> &vertices) {
    // DT of vertices
    TetMesh mesh(true);
    TetMeshTetrahedrizer dt(mesh);
    dt.mesh->vertices.resize(vertices.size());
    size_t i = 0;
    for (uint32_t v : vertices)
        dt.mesh->vertices[i++] = delmesh.mesh->vertices[v];
    dt.mesh->inc_tet.resize(vertices.size(), UINT64_MAX);
    dt.tetrahedrize();

    uint64_t nt[2];
    for (size_t i = 0; i < vertices.size(); i++)
        v_reindex[vertices[i]] = (uint32_t)i;
    for (uint64_t t : bnd) {
        uint32_t v[3];
        delmesh.mesh->getFaceVertices(t, v);
        for (int i = 0; i < 3; i++)
        ...
        if (!dt.mesh->getTetsFromFaceVertices(v[0], v[1], v[2], nt)) {
            for (size_t i = 0; i < vertices.size(); i++)
                v_reindex[vertices[i]] = UINT32_MAX;
...
uint64_t PLCx::meshCavity(const std::vector<uint64_t> &bnd,
        const std::vector<uint32_t> &vertices,
    ...
    // Swap ghosts to tail
    uint32_t last = dt.mesh->numTets() - 1;
    while (dt.mesh->isGhost(last))
        last--;
    for (uint64_t t = 0; t < last; t++)
        if (dt.mesh->isGhost(t)) {
            dt.mesh->swapTets(t, last--);
            while (dt.mesh->isGhost(last))
                last--;
        }

    // Identify and mark bnd_faces in DT
    //   for each triplet v1,v2,v3, find in VT(v1) the two tets that share v2 and
    //   v3 and mark their opposite corners.
    std::vector<bool> cornerMask(dt.mesh->tet_node.size(), false);
    uint64_t nt[2];
    ...
    for (uint64_t t : bnd) {
        ...
        const uint64_t c1 = dt.mesh->getCornerFromOppositeTet(nt[0], nt[1]);
        const uint64_t c2 = dt.mesh->getCornerFromOppositeTet(nt[1], nt[0]);
        cornerMask[c1] = true;
        cornerMask[c2] = true;
    ...
    for (i = 0; i < dt.mesh->numTets(); i++)
        dt.mesh->mark_tetrahedra[i] = DT_UNKNOWN; // Might be redundant... CHECK

    // Make vector C of classified tets
    std::vector<uint64_t> C;

    // Find a ghost tet t in DT that has at least a vertex with v_orient!=0
    for (i = 0; i < dt.mesh->tet_node.size(); i += 4)
        if (dt.mesh->tet_node[i + 3] == INFINITE_VERTEX &&
                (v_orient[vertices[dt.mesh->tet_node[i]]] ||
                 v_orient[vertices[dt.mesh->tet_node[i + 1]]] ||
                 v_orient[vertices[dt.mesh->tet_node[i + 2]]]))
            break;
    assert(i < dt.mesh->tet_node.size());

    // Mark internal tets starting from i
    dt.markInnerTets(cornerMask, i >> 2);

    std::vector<uint64_t> remap;
    remap.resize(dt.mesh->numTets());
    for (size_t i = 0; i < remap.size(); i++)
        remap[i] = i;

    // Swap DT_OUT/ghost tets to tail
    last = dt.mesh->numTets() - 1;
    while (dt.mesh->mark_tetrahedra[last] == DT_OUT)
        last--;
    for (uint64_t t = 0; t < last; t++)
        if (dt.mesh->mark_tetrahedra[t] == DT_OUT) {
    ...
    // Transform DT_OUT tets to ghosts and make sure that all vt* point to DT_IN
    // tets
    uint64_t num_in_tets = 0;
    for (size_t i = 0; i < dt.mesh->numTets(); i++) {
        if (dt.mesh->mark_tetrahedra[i] == DT_OUT) {
            dt.mesh->tet_node[i * 4 + 3] = INFINITE_VERTEX;
        } else {
            num_in_tets++;
            assert(dt.mesh->mark_tetrahedra[i] == DT_IN);
            dt.mesh->inc_tet[dt.mesh->tet_node[i * 4 + 0]] =
                dt.mesh->inc_tet[dt.mesh->tet_node[i * 4 + 1]] =
                dt.mesh->inc_tet[dt.mesh->tet_node[i * 4 + 2]] =
                dt.mesh->inc_tet[dt.mesh->tet_node[i * 4 + 3]] = i;
        }
    }
    ...
    for (size_t i = 0; i < dt.mesh->tet_neigh.size(); i++)
        dt.mesh->tet_neigh[i] += delmesh.mesh->tet_neigh.size();

    // 3) Update inc_tets
    for (size_t i = 0; i < dt.mesh->vertices.size(); i++)
        dt.mesh->inc_tet[i] += delmesh.mesh->numTets();

    // 4) Remap dt.tet_node so that they point to IDs in delmesh
    //    (if != INF_VERTEX dt.tet_node[i] = vertices[dt.tet_node[i]])
    //
    for (size_t i = 0; i < dt.mesh->tet_node.size(); i++)
        if (dt.mesh->tet_node[i] != INFINITE_VERTEX)
            dt.mesh->tet_node[i] = vertices[dt.mesh->tet_node[i]];

    size_t oNumTets = delmesh.mesh->numTets();
    num_in_tets += oNumTets;
    for (size_t i = 0; i < dt.mesh->numVertices(); i++) {
        if (dt.mesh->inc_tet[i] < num_in_tets)
            delmesh.mesh->inc_tet[vertices[i]] = dt.mesh->inc_tet[i];
    }

    delmesh.mesh->tet_node.insert(delmesh.mesh->tet_node.end(),
            dt.mesh->tet_node.begin(),
            dt.mesh->tet_node.end());
    delmesh.mesh->tet_neigh.insert(delmesh.mesh->tet_neigh.end(),
            dt.mesh->tet_neigh.begin(),
            dt.mesh->tet_neigh.end());
    delmesh.mesh->mark_tetrahedra.resize(delmesh.mesh->mark_tetrahedra.size() +
            dt.mesh->mark_tetrahedra.size(),
            0);

    size_t on4 = oNumTets * 4;
    for (bdUpdater &b : bdpairs) {
        if (dt.mesh->mark_tetrahedra[b.t1 >> 2] == DT_IN) {
            delmesh.mesh->tet_neigh[on4 + b.t1] = b.bnd;
            delmesh.mesh->tet_neigh[b.bnd] = on4 + b.t1;
        } else {
            assert(dt.mesh->mark_tetrahedra[b.t2 >> 2] == DT_IN);
            delmesh.mesh->tet_neigh[on4 + b.t2] = b.bnd;
            delmesh.mesh->tet_neigh[b.bnd] = on4 + b.t2;
        }
    }

    // Delete mesh tail
    last = delmesh.mesh->numTets();
    while (delmesh.mesh->isGhost(--last))
        ;
    last++;
    delmesh.mesh->tet_node.resize(last * 4);
    delmesh.mesh->tet_neigh.resize(last * 4);
    ...
    for (size_t i = on4; i < delmesh.mesh->tet_node.size(); i++) {
        if (delmesh.mesh->tet_neigh[i] >= delmesh.mesh->tet_node.size()) {
            base.push_back(i);
            uint32_t w[3];
            delmesh.mesh->getFaceVertices(i, w);
