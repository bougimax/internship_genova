...
// This is a generic point. It can be extended as either explicit or implicit
// point
class genericPoint {
protected:
  Point_Type type;
...
public:
  ...
  //
  // I do not see how an optimizer may spoil these simple functions or code
  // around them, but if you experience strange behaviour you may try replacing
  // the type casts with memcpy to an object and hope that the optimizer
  // recognizes that an actual copy is not necessary.
  ...
  const class explicitPoint2D &toExplicit2D() const {
    return (explicitPoint2D &)(*this);
  }
  const class implicitPoint2D_SSI &toSSI() const {
    return (implicitPoint2D_SSI &)(*this);
  }
  const class explicitPoint3D &toExplicit3D() const {
    return (explicitPoint3D &)(*this);
  }
  const class implicitPoint3D_LPI &toLPI() const {
    return (implicitPoint3D_LPI &)(*this);
  }
  const class implicitPoint3D_TPI &toTPI() const {
    return (implicitPoint3D_TPI &)(*this);
  }
  const class implicitPoint3D_LNC &toLNC() const {
    return (implicitPoint3D_LNC &)(*this);
  }

  // Calculates the first two cartesian coordinates. If the point is implicit,
  // these coordinates are approximated due to floating point roundoff. If
  // apap==true, the approximation is as precise as possible (slightly slower).
  ...
  // If apap==true, the approximation is as precise as possible (slightly
  // slower). Returns 0 if the point is not 3D or if the implicit point is
  // undefined.
  bool getApproxXYZCoordinates(double &x, double &y, double &z,
                               bool apap = false) const;

  // Calculates the two/three cartesian coordinates exactly.
  bool getExactXYCoordinates(bigrational &x, bigrational &y) const;
  bool getExactXYZCoordinates(bigrational &x, bigrational &y,
                              bigrational &z) const;

  std::string get_str() const {
    bigrational x, y, z;
    if (!getExactXYZCoordinates(x, y, z))
      return "UNDEFINED_GENERIC_POINT";
    return x.get_str() + " " + y.get_str() + " " + z.get_str();
  }

  // These are the indirect predicates supported up to now.
  // In each predicate, it is assumed that input points are either all 2D or all
  // 3D as expected. No check is performed. Passing wrong implicit points may
  // result in unpredictable behaviour.

  // Orient2D - fully supported
  // Input points can be any combination of 2D points.
  static int orient2D(const genericPoint &a, const genericPoint &b,
  ...
  static int orient2Dxy(const genericPoint &a, const genericPoint &b,
                        const genericPoint &c);
  static int orient2Dyz(const genericPoint &a, const genericPoint &b,
                        const genericPoint &c);
  static int orient2Dzx(const genericPoint &a, const genericPoint &b,
                        const genericPoint &c);

  // Orient3D - fully supported
  // Input can be any combination of 3D points
  static int orient3D(const genericPoint &a, const genericPoint &b,
                      const genericPoint &c, const genericPoint &d);

  // InSphere - fully supported
  // Input can be any combination of 3D points
  static int inSphere(const genericPoint &a, const genericPoint &b,
                      const genericPoint &c, const genericPoint &d,
                      const genericPoint &e);

  // incircle - fully supported
  // Input can be any combination of 2D points
  static int incircle(const genericPoint &a, const genericPoint &b,
                      const genericPoint &c, const genericPoint &d);

  // incirclexy - fully supported
  // Input can be any combination of 3D points
  static int incirclexy(const genericPoint &a, const genericPoint &b,
                        const genericPoint &c, const genericPoint &d);

  // Sign of (a-c) dot (b-c)
  static int dotProductSign2D(const genericPoint &a, const genericPoint &b,
                              const genericPoint &c);
  static int dotProductSign3D(const genericPoint &a, const genericPoint &b,
                              const genericPoint &c);
  ...
  static bool coincident(const genericPoint &a, const genericPoint &b) {
  ...
  // 2 - if m == |z|
  //
  // Warning: this function assumes that the triangle is not exactly degenerate.
  // It may crash otherwise.
  static int maxComponentInTriangleNormal(double v1x, double v1y, double v1z,
                                          double v2x, double v2y, double v2z,
                                          double v3x, double v3y, double v3z);

  // TRUE if A-B-C are not collinear
  static bool misaligned(const genericPoint &A, const genericPoint &B,
                         const genericPoint &C) {
    return (orient2Dxy(A, B, C) || orient2Dyz(A, B, C) || orient2Dzx(A, B, C));
  }

  // TRUE if 'p' is in the interior of v1-v2
  static bool pointInInnerSegment(const genericPoint &p, const genericPoint &v1,
                                  const genericPoint &v2);

  // TRUE if 'p' is in the closure of v1-v2
  static bool pointInSegment(const genericPoint &p, const genericPoint &v1,
                             const genericPoint &v2);

  // TRUE if P is in the interior of <A,B,C>
  // Points are assumed to be coplanar. Undetermined otherwise.
  static bool pointInInnerTriangle(const genericPoint &P, const genericPoint &A,
                                   const genericPoint &B,
                                   const genericPoint &C);

  // TRUE if P is in the closure of <A,B,C>
  // Points are assumed to be coplanar. Undetermined otherwise.
  static bool pointInTriangle(const genericPoint &P, const genericPoint &A,
                              const genericPoint &B, const genericPoint &C);
  // Same as above, but this version initializes oAB, oAC and oCA with the
  // orientation of P wrt one of the edges (0 = on edge)
  static bool pointInTriangle(const genericPoint &P, const genericPoint &A,
                              const genericPoint &B, const genericPoint &C,
                              int &oAB, int &oBC, int &oCA);

  // TRUE if the interior of A-B intersects the interior of P-Q at a single
  // point Points are assumed to be coplanar. Undetermined otherwise.
  static bool innerSegmentsCross(const genericPoint &A, const genericPoint &B,
                                 const genericPoint &P, const genericPoint &Q);

  // TRUE if the closure of A-B intersects the closure of P-Q at a single point
  // Points are assumed to be coplanar. Undetermined otherwise.
  static bool segmentsCross(const genericPoint &A, const genericPoint &B,
                            const genericPoint &P, const genericPoint &Q);

  // TRUE if interior of s1-s2 intersects interior of <v1,v2,v3> at a single
  // point
  static bool innerSegmentCrossesInnerTriangle(const genericPoint &s1,
                                               const genericPoint &s2,
                                               const genericPoint &v1,
                                               const genericPoint &v2,
                                               const genericPoint &v3);

  // TRUE if the infinite straight line by s1-s2 intersects triangle <v1,v2,v3>
  // at a single internal point
  static bool lineCrossesInnerTriangle(const genericPoint &s1,
                                       const genericPoint &s2,
                                       const genericPoint &v1,
                                       const genericPoint &v2,
                                       const genericPoint &v3);
  ...
  // TRUE if the infinite straight line by s1-s2 intersects triangle <v1,v2,v3>
  // at a single point
  static bool lineCrossesTriangle(const genericPoint &s1,
                                  const genericPoint &s2,
                                  const genericPoint &v1,
                                  const genericPoint &v2,
                                  const genericPoint &v3);
  ...
  static bool innerSegmentCrossesTriangle(const genericPoint &s1,
                                          const genericPoint &s2,
                                          const genericPoint &v1,
                                          const genericPoint &v2,
                                          const genericPoint &v3);

  // The following methods are equivalent to the corresponding functions
  // hereabove, but faster. They assume that points are coplanar and the
  // dominant normal component is n_max (see maxComponentInTriangleNormal()).
  static int orient2D(const genericPoint &a, const genericPoint &b,
                      const genericPoint &c, int n_max) {
    if (n_max == 0)
  ...
  static bool misaligned(const genericPoint &A, const genericPoint &B,
                         const genericPoint &C, int n_max) {
    return ((n_max == 2 && orient2Dxy(A, B, C)) ||
            (n_max == 0 && orient2Dyz(A, B, C)) ||
            (n_max == 1 && orient2Dzx(A, B, C)));
  }

  static bool pointInInnerSegment(const genericPoint &p, const genericPoint &v1,
                                  const genericPoint &v2, int n_max);
  static bool pointInSegment(const genericPoint &p, const genericPoint &v1,
                             const genericPoint &v2, int n_max);
  static bool pointInInnerTriangle(const genericPoint &P, const genericPoint &A,
                                   const genericPoint &B, const genericPoint &C,
  ...
  bool getIntervalLambda(interval_number &lx, interval_number &ly,
                         interval_number &d) const;
  void getExactLambda(double **lx, int &lxl, double **ly, int &lyl, double **d,
                      int &dl) const;
  void getBigfloatLambda(bigfloat &lx, bigfloat &ly, bigfloat &d) const;
  bool getIntervalLambda(interval_number &lx, interval_number &ly,
                         interval_number &lz, interval_number &d) const;
  void getExactLambda(double **lx, int &lxl, double **ly, int &lyl, double **lz,
                      int &lzl, double **d, int &dl) const;
  void getBigfloatLambda(bigfloat &lx, bigfloat &ly, bigfloat &lz,
                         bigfloat &d) const;
};
...
public:
  ...
  explicitPoint2D(double _x, double _y)
      : genericPoint(Point_Type::EXPLICIT2D), x(_x), y(_y) {}
  explicitPoint2D(const explicitPoint2D &b)
      : genericPoint(Point_Type::EXPLICIT2D), x(b.x), y(b.y) {}

  void operator=(const explicitPoint2D &b) {
    type = Point_Type::EXPLICIT2D;
    x = b.x;
    y = b.y;
  }
  bool operator==(const explicitPoint2D &e) const {
    return x == e.x && y == e.y;
  }
  ...
  bool getExactXYCoordinates(bigrational &_x, bigrational &_y) const {
    _x = bigfloat(x);
    _y = bigfloat(y);
    return true;
  }
...
private: // Cached values
  mutable interval_number dfilter_lambda_x, dfilter_lambda_y,
      dfilter_denominator;
  bool needsIntervalLambda() const {
    return (dfilter_denominator.isNAN());
  } // TRUE if NAN
...
public:
  ...
  explicitPoint3D(double _x, double _y, double _z)
      : genericPoint(Point_Type::EXPLICIT3D), x(_x), y(_y), z(_z) {}
  explicitPoint3D(const explicitPoint3D &b)
      : genericPoint(Point_Type::EXPLICIT3D), x(b.x), y(b.y), z(b.z) {}

  void operator=(const explicitPoint3D &b) {
    type = Point_Type::EXPLICIT3D;
    x = b.x;
    y = b.y;
    z = b.z;
  }
  bool operator==(const explicitPoint3D &e) const {
  ...
  bool getExactXYZCoordinates(bigrational &_x, bigrational &_y,
                              bigrational &_z) const {
    _x = bigfloat(x);
    _y = bigfloat(y);
    _z = bigfloat(z);
    return true;
...
public:
  implicitPoint3D_LPI(const explicitPoint3D &_p, const explicitPoint3D &_q,
                      const explicitPoint3D &_r, const explicitPoint3D &_s,
...
private: // Cached values
  interval_number dfilter_lambda_x, dfilter_lambda_y, dfilter_lambda_z,
      dfilter_denominator;
...
public:
  implicitPoint3D_TPI(const explicitPoint3D &_v1, const explicitPoint3D &_v2,
